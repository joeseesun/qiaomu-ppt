...
- NotebookLM-native PPTX is a separate explicit route, not the normal editable PPTX route. It is allowed to be image-backed only when `export_manifest.json` records `route: notebooklm_native_slide_deck` or `formats.pptx.image_backed_ok: true`, and `pptx_text_check.json` records `allow_image_backed: true`.
- NotebookLM-native projects must include `notebooklm_generation_manifest.json`, `notebooklm_slide_prompt.txt`, and `reports/notebooklm_watermark_cleanup.json`. Prefer official account-level watermark removal when available. `scripts/strip_notebooklm_watermark.py` may remove editable watermark shapes or small lower-right logo objects, but if the watermark is baked into full-slide raster images it must report that limitation instead of pretending a clean artifact was created.
