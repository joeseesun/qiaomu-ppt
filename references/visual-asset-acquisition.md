...
The thesis is simple: do not generate whole slides as images. Generate visual
assets under a locked deck spec, then keep titles, body text, captions, charts,
tables, citations, and layout objects editable in SVG/PPT/HTML foreground.
...
|---|---|---|---|
| `ai` | `Pending` | `Generated`, `Needs-Manual`, `Missing`, `Failed` | atmosphere, chapter art, concept metaphor, scenario illustration, object cutaway, texture |
...
3. Slide claim/proof/audience-state `content_link`.
4. `background_duty` and `semantic_anchor`.
5. Composition primitive or local visual type.
6. Specific subject using concrete nouns.
7. Container note: size, aspect ratio, page role, and overlay reservation only
...
Qiaomu uses a three-path strategy:

1. **Path A: configured API backend**  
   Use a configured image backend such as `gpt-image-2` when available. The
   prompt source of truth is `assets/images/image_prompts.json`.

2. **Path B: host-native image tool**  
