...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description='Qiaomu PPT - SVG to PPTX Tool (Office Compatibility Mode)',
...
''',
    ...
    try:
        ...
        if exc.name == 'pptx':
            print(
                "Error: python-pptx is required for SVG-to-PPTX export. "
                "Install qiaomu-ppt requirements first: python3 -m pip install -r requirements.txt",
                file=sys.stderr,
            )
            return 1
        raise
    ...
    if gen_native:
        if verbose:
            print("Qiaomu PPT - SVG to PPTX Tool")
            print("=" * 50)
    ...
    if gen_legacy:
        if verbose:
            if gen_native:
                print()
                print("-" * 50)
            print("Qiaomu PPT - SVG to PPTX Tool (SVG Reference)")
            print("=" * 50)
            print(f"  Project path: {project_path}")
