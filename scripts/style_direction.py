...
def build_direction(project: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "style_fit_decision": {
            "aesthetic_goal": "Make the deck look inevitable for the topic, audience, evidence, and desired action.",
            "primary_visual_family": style.get("id"),
            "alternative_family": "",
            "avoid_family": "",
            "avoid_reason": "Record a concrete avoid family before final rendering, especially when the user did not specify a visual style.",
            "domain_fit_reason": "Style must be selected for topic, audience, evidence type, credibility needs, and intended use; not for novelty or personal taste.",
            "audience_fit_reason": "Record how the style serves the audience and venue.",
            "visual_temperature": "calm / premium / rigorous / energetic / playful / cinematic / documentary",
            "rhythm_strategy": "Mix anchor, dense, diagram, breathing, and action pages so the deck does not repeat one visual fingerprint.",
            "image_content_binding_policy": "Every generated/source image must bind to the slide claim, proof object, focus region, annotation targets, safe text area, and integration move.",
        },
        "density_targets": targets,
        "asset_counts": counts,
        "layout_program": layout_program(layout_recs, slides),
        "non_negotiables": [
            "When the user does not specify a style, choose the style that best fits the content domain, audience, evidence type, credibility needs, visual temperature, venue, and intended action.",
            "Record a concrete avoid_family/avoid_reason for this deck; a pretty style that weakens credibility, clarity, emotion, or reading should fail.",
            "Do not hard-code the previous task's style rules into every new topic; generate domain-specific avoid rules for the current subject.",
            "Generated image prompts must name the page-specific object, scene, artifact, system, metric, person, place, or decision that foreground text can annotate.",
        ...
        "failure_signals": [
            ...
            "default style chosen by taste while ignoring content-domain credibility",
            "generated images that could be reused unchanged under unrelated slide titles",
            "a deck inherits the previous task's aesthetic instead of making a fresh topic/audience/media decision",
            "the selected style makes the subject less clear, less trustworthy, less emotionally accurate, or harder to read",
        ],
        "warnings": warnings,
        "artifacts": {
...
def render_markdown(direction: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Style Fit Decision",
        "",
        f"- Primary family: `{direction.get('style_fit_decision', {}).get('primary_visual_family', '')}`",
        f"- Avoid family: `{direction.get('style_fit_decision', {}).get('avoid_family', '')}`",
        f"- Domain fit: {direction.get('style_fit_decision', {}).get('domain_fit_reason', '')}",
        f"- Audience fit: {direction.get('style_fit_decision', {}).get('audience_fit_reason', '')}",
        f"- Visual temperature: {direction.get('style_fit_decision', {}).get('visual_temperature', '')}",
        f"- Rhythm: {direction.get('style_fit_decision', {}).get('rhythm_strategy', '')}",
        f"- Image binding: {direction.get('style_fit_decision', {}).get('image_content_binding_policy', '')}",
        "",
        "## Non-Negotiables",
        "",
    ]
