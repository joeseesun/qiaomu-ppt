...
def _encode_pil_to_data_uri(
    ...
    *,
    ...
    # fall back to those.
    chosen = encoded_bytes or fallback_bytes
    if fallback_bytes and encoded_bytes and len(fallback_bytes) < len(encoded_bytes):
        chosen = fallback_bytes

    effective_max_dimension = max_dimension
    if auto_max_dimension:
        effective_max_dimension = (
            min(max_dimension, auto_max_dimension)
            if max_dimension
            else auto_max_dimension
        )

    try:
        chosen = _optimize_image_bytes(
            chosen, mime_type, compress=compress, max_dimension=effective_max_dimension,
        )
    except (OSError, ValueError, SystemError):
        if not fallback_bytes:
            return None
...
def _process_one_image(
    ...
    *,
    ...
    auto_max_dimension: int | None = None
    if auto_downsample:
        visible_long_edge = max(new_w, new_h)
        if visible_long_edge > 0 and downsample_scale > 0:
            auto_max_dimension = max(
                downsample_min_dimension,
                int(math.ceil(visible_long_edge * downsample_scale)),
            )
    ...
    encoded = _encode_pil_to_data_uri(
        ...
        auto_max_dimension=auto_max_dimension,
        fallback_bytes=raw_bytes if not transformed else None,
    )
    if encoded is None and transformed:
        # Some malformed or extremely small rasters can fail after an in-memory
        # crop/resize even though their original bytes are still embeddable.
        # Keep production moving by embedding the original image in the
        # declared box rather than failing the whole deck.
        final_img = img
        new_x, new_y, new_w, new_h = box_x, box_y, box_w, box_h
        encoded = _encode_pil_to_data_uri(
            final_img,
...
def align_and_embed_images_in_svg(
    ...
    *,
    ...
    downsample_scale: float = 2.5,
    downsample_min_dimension: int = 960,
) -> tuple[int, int]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--auto-downsample', action='store_true',
                        help='Downsample raster images from their SVG display size before embedding')
    parser.add_argument('--downsample-scale', type=float, default=2.5,
                        help='Pixel multiplier over the final SVG display box for --auto-downsample')
    parser.add_argument('--downsample-min-dimension', type=int, default=960,
                        help='Minimum long edge used by --auto-downsample')
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    proc, err = align_and_embed_images_in_svg(
        ...
        auto_downsample=args.auto_downsample,
        downsample_scale=args.downsample_scale,
        downsample_min_dimension=args.downsample_min_dimension,
    )
