#!/usr/bin/env python3
"""
Qiaomu PPT - SVG Post-processing Tool (Unified Entry Point)
...
def finalize_project(
    ...
    auto_downsample_images: bool = True,
    ...
    Args:
        ...
        auto_downsample_images: Downsample embedded rasters from their final SVG display box
        downsample_scale: Pixel multiplier over the visible display box
        downsample_min_dimension: Minimum long edge for auto-downsampled images
    """
    ...
    if options.get('align_images'):
        ...
        for svg_file in svg_final.glob('*.svg'):
            ...
            count, errs = align_and_embed_images_in_svg(
                ...
                auto_downsample=auto_downsample_images,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Qiaomu PPT - SVG Post-processing Tool',
...
Aliases (still accepted):
  crop-images, fix-aspect, embed-images  → all map to align-images
    ...
    parser.add_argument('--no-auto-downsample-images', action='store_true',
                        help='Disable SVG-display-size-aware raster downsampling before embedding')
    parser.add_argument('--downsample-scale', type=float, default=2.5,
                        help='Pixel multiplier over final SVG display box for automatic image downsampling (default: 2.5)')
    parser.add_argument('--downsample-min-dimension', type=int, default=960,
                        help='Minimum long edge for automatic image downsampling (default: 960)')
    ...
    success = finalize_project(args.project_dir, options, args.dry_run, args.quiet,
                               ...
                               auto_downsample_images=not args.no_auto_downsample_images,
