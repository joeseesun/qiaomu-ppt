...
4. Lock the visual system.
   ...
   - In Codex environments where the built-in image generation tool is available, use the prompt pack to generate 3-5 quiet 16:9 text-free, atmosphere-only background bitmap assets before rendering. Background images may contain color fields, gradients, soft light, subtle grain, and abstract decoration, but must not contain boxes, panels, frames, cards, placeholders, chart areas, image slots, UI chrome, or text. Titles, cards, charts, frames, and all page elements must be added later as editable foreground objects. If image generation is unavailable, use neutral surfaces only; do not fake richness with decorative lines or baked-in layout containers.
...
6. Run quality gates before calling the work done.
   ...
   - Reject decorative background lines unless they are functional chart axes, table rules, connectors, or intentionally editable foreground separators. Lines that only add "tech feel" are defects.
